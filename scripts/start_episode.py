...
def build_run_identity(artist_slug: str, release: bool, settings: dict[str, str], artist_rows: list[list[str]], episode_rows: list[list[str]]) -> tuple[str, str, str]:
    if release:
        episode_id = next_episode_id(settings, artist_rows, episode_rows)
        episode_slug = f"episode{episode_id}_{artist_slug}"
        run_type = "release"
        return episode_id, episode_slug, run_type
    episode_id = f"test_{artist_slug}"
...
def reserve_redirects(token: str, episode_id: str, artist_name: str, artist_slug: str, ts: str, episode_slug: str) -> dict[str, str]:
    ...
    seeds = [
        ("video", "episode_video", f"{episode_slug}_video"),
        ("playlist", "spotify_playlist", f"{episode_slug}_playlist"),
        ("painting", "painting1", f"{episode_slug}_painting1"),
        ("painting", "painting2", f"{episode_slug}_painting2"),
        ("painting", "painting3", f"{episode_slug}_painting3"),
    ]
...
def create_workspace(episode_slug: str) -> tuple[Path, Path]:
    episode_dir = episode_slug
...
def write_manifest(
    ...
    manifest_path = output_dir / "publish" / f"{episode_slug}_start-manifest.json"
...
def parse_nb_blocks(text: str) -> list[tuple[str, str, str]]:
    pattern = re.compile(
        r"(?:^|\n)(?:#+\s*)?(?:\*\*)?PAINTING\s+([123])\s+·\s+([A-Z]+)(?:\*\*)?\s*[\r\n]+"
        r"(?:\*\*)?TITLE:(?:\*\*)?\s*(.+?)\s*[\r\n]+"
        r"(?:\*\*)?PROMPT:(?:\*\*)?\s*(.*?)(?=(?:\n---\n|\n(?:#+\s*)?(?:\*\*)?PAINTING\s+[123]\s+·|\Z))",
        re.DOTALL,
    )
...
def generate_adna_and_nb(artist_name: str, artist_slug: str, episode_slug: str, output_dir: Path) -> dict[str, str]:
    ...
    adna_path = output_dir / "adna" / f"{episode_slug}_ADNA-text.txt"
    ...
    for index, title, prompt in nb_blocks:
        ...
        path = output_dir / "nb" / f"{episode_slug}_NB_Painting{index}_{title_slug}.txt"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--test", action="store_true", help="Run as test_<artist_slug> without consuming release numbering.")
    parser.add_argument("--release", action="store_true", help="Run as numbered release episodeNNN_<artist_slug>.")
...
def main() -> None:
    ...
    ts = now_iso()
    release_mode = args.release and not args.test

    artist_slug_probe = slugify(artist_name)
    episode_id, episode_slug, run_type = build_run_identity(
        artist_slug_probe,
    ...
    if release_mode:
        ...
        episode_slug = f"test_{artist_slug}"
        episode_id = episode_slug

    input_dir, output_dir = create_workspace(episode_slug)
    redirects = reserve_redirects(token, episode_id, artist_name, artist_slug, ts, episode_slug)
    append_episode_row(token, episode_id, artist_name, artist_slug, episode_slug, run_type, input_dir, output_dir, redirects, ts)
    manifest_path = write_manifest(episode_id, episode_slug, artist_name, artist_slug, run_type, input_dir, output_dir, redirects, ts)

    artifact_data = generate_adna_and_nb(artist_name, artist_slug, episode_slug, output_dir)
    ...
    print(f"Episode started: {episode_slug}")
